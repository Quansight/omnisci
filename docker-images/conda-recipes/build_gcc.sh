...
urls="http://mirrors-usa.go-parts.com/gcc/releases/gcc-${GCC_VER}/gcc-${GCC_VER}.tar.bz2 \
         ...
         http://www.mpfr.org/mpfr-${MPFR_VER}/mpfr-${MPFR_VER}.tar.bz2 \
...
scl enable ${DEVTOOLSET} './configure --prefix=/usr/local \
                                     --enable-plugin     \
                                     --with-sysroot=/    \
...
scl enable ${DEVTOOLSET} '/gcc-build/configure \
      ...
      --with-tune=generic'
scl enable ${DEVTOOLSET} 'make -j$(getconf _NPROCESSORS_ONLN) bootstrap-lean'
scl enable ${DEVTOOLSET} 'make install-strip'
